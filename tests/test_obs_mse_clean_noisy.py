...
def test_sparse_forward_does_not_report_solution_observation_mse():
    registry = build_default_registry()
    raw = registry.synthetic_raw("poisson", n=1, resolution=8)
    batch = registry.make_task(raw, "poisson", "sparse_forward", num_sensors=8, noise_level=0.0, seed=1)
    pred = batch.target_fields.clone()
    assert math.isnan(_obs_mse_clean(pred, batch.target_fields, batch))
    assert math.isnan(_obs_mse_noisy(pred, batch))
