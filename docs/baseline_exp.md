...
续跑身份按单个 baseline 隔离：使用该 baseline 合并后的最终方法配置哈希、共享/本 baseline/对应 vendored 官方实现的代码依赖哈希、稳定的数据内容哈希，以及矩阵中显式的任务、预算和协议字段。完整 YAML 哈希与 Git revision 继续记录用于审计，但仅修改其他 baseline 的配置或代码不得使本 baseline 的完成结果失效。旧结果若没有这三项语义哈希，尤其来自 dirty worktree 时无法安全反推，需完成一次迁移重跑；之后可按新身份稳定续跑。

监督方法的训练适配保留以下官方核心设置：FNO 使用 NeuralOperator 的 H1 loss、AdamW、weight decay 与 StepLR；DeepONet 使用 DeepXDE 网络及 Adam/MSE；RecFNO 使用 L1、Adam 与 ExponentialLR；Senseiver 使用 sum-MSE、Adam 与 train-loss 早停；VoronoiCNN 使用七层 7x7 卷积栈、Adam/MSE 与 validation-loss 最佳 checkpoint。为减少无效训练，Senseiver 和 VoronoiCNN 的早停 patience 统一适配为 20。iFNO 明确记录 iFNO 预训练、VAE 预训练、联合训练三个请求预算和实际完成预算，Darcy 顺序为 iFNO→VAE→joint，Navier-Stokes 顺序为 VAE→iFNO→joint，并保留 VAE 四倍几何增强。正式训练采用总计 100 epoch 的三阶段上限，分别分配为 iFNO 预训练 50、VAE 预训练 15、联合训练 35。三个阶段分别执行验证集早停并恢复各自最佳权重：iFNO 预训练为 `min_epochs=3`、`patience=3`，VAE 预训练为 `min_epochs=4`、`patience=3`，联合训练为 `min_epochs=2`、`patience=2`，下降阈值均为 `1e-4`。VAE 的验证损失关闭几何增强和随机潜变量采样，避免随机性影响停止判定。这些是 FM4PDE 在官方三阶段配方上的计算预算适配。由于该训练已同时优化正向与反向映射，每个 PDE/seed 只在 Full Forward 行训练一次；Full Inverse 行为 `eval_only`，复用同一 checkpoint（含 VAE），并按反向任务交换 x/y 归一化统计，不得再次调用 `fit()`。
