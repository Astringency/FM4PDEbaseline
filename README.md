...
`runs/main_results`. Burgers is not eligible
...
separate rows with their condition and ablation name recorded. The workbook
keeps PDE, method, task, distribution, condition, sensor mode, seed, and
ablation settings, followed by adjacent mean and standard-deviation columns
for `rel L2(a)`, `rel L2(u)`, `joint rel L2`, and `pde L` (for joint tasks).
Standard deviations come directly from each evaluation's `summary.json`;
missing or non-finite values stay blank. Source paths and validation metadata
are omitted from the workbook.
